# Xiyue - Pixel Creation Workbench
...
## Xiyue Workbench Update

- **Modern pixel workbench**: Warm canvas, navy navigation, brick-red primary actions, 2px hard borders, and zero-blur shadows, while preserving AI-first categories, search, recents, favorites, and the right-side asset drawer.
- **Unified pixel icons**: All 20 tools, six categories, and common actions share local 16×16 SVG-grid icons with no downloaded fonts or large visual assets.
- **Xiyue Workshop**: All former advanced-workshop slicing, scaling, and workflow tools remain available; internal names and old storage keys stay compatible.
- **Security and performance**: Session-only API keys by default, streamed uploads, real media validation, persistent RQ IDs, and lazy-loaded tools.
- **Shortcuts**: `Ctrl/Cmd+K` opens search and `Esc` returns to the workbench. Conflicting single-key shortcuts were removed.
- **Responsive layout**: Desktop keeps the 240px sidebar and the two-column API form/history view; below 900px navigation becomes a drawer, and 390px uses a single column without horizontal overflow.
...
- **Sprite Sheet Adjust**: Split preview, frame selection, animation preview, frame navigation (buttons + **A/D**), and recombine export after offsets
...
### Xiyue Workshop (efficiency)
- **Xiyue Workshop**: Local **Custom Scale**, **Custom Slice**, **Unify Size**, **Sheet Pro**, and more, with no login required.
...
- **Ctrl / Cmd + K**: Open global tool search.
- **Esc**: Close an overlay or return from a top-level tool to the workbench.
...
1. Open https://github.com/User010117/xiyue/settings/pages
2. Under **Build and deployment**, set **Source** to **GitHub Actions**
3. Save. Future pushes to `main` will auto-deploy

**URL:** https://User010117.github.io/xiyue/
...
| [DEV_PLAN_extensions.md](./DEV_PLAN_extensions.md) | Extension plan and shipped checklist |
