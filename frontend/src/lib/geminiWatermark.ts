/**
 * Gemini 可见角标处理：通过反向 Alpha 混合恢复角标覆盖前的像素。
 * 算法参考：https://github.com/allenk/GeminiWatermarkTool
 * 公式：watermarked = alpha * logo + (1-alpha) * original
 *       original = (watermarked - alpha * logo) / (1-alpha)
 */

export type WatermarkSize = 48 | 96

/** 手动修复区域使用原图像素坐标，便于画布缩放后仍得到稳定结果。 */
export interface WatermarkRepairRect {
  x: number
  y: number
  width: number
  height: number
}

/**
 * 用矩形四周的原始像素向内插值，覆盖非标准位置的可见水印。
 * ponytail: 该轻量算法只适合纯色或缓变背景；复杂纹理需要真正的图像修复模型。
 */
export function repairWatermarkFromEdges(
  imageData: Pick<ImageData, 'data' | 'width' | 'height'>,
  rect: WatermarkRepairRect,
): void {
  const { data, width: imageWidth, height: imageHeight } = imageData
  const x = Math.max(1, Math.min(imageWidth - 2, Math.round(rect.x)))
  const y = Math.max(1, Math.min(imageHeight - 2, Math.round(rect.y)))
  const width = Math.max(1, Math.min(imageWidth - x - 1, Math.round(rect.width)))
  const height = Math.max(1, Math.min(imageHeight - y - 1, Math.round(rect.height)))

  for (let py = y; py < y + height; py++) {
    const verticalRatio = (py - y + 1) / (height + 1)
    for (let px = x; px < x + width; px++) {
      const horizontalRatio = (px - x + 1) / (width + 1)
      const outputIndex = (py * imageWidth + px) * 4
      const leftIndex = (py * imageWidth + x - 1) * 4
      const rightIndex = (py * imageWidth + x + width) * 4
      const topIndex = ((y - 1) * imageWidth + px) * 4
      const bottomIndex = ((y + height) * imageWidth + px) * 4

      for (let channel = 0; channel < 4; channel++) {
        const horizontal = (data[leftIndex + channel] ?? 0) * (1 - horizontalRatio)
          + (data[rightIndex + channel] ?? 0) * horizontalRatio
        const vertical = (data[topIndex + channel] ?? 0) * (1 - verticalRatio)
          + (data[bottomIndex + channel] ?? 0) * verticalRatio
        data[outputIndex + channel] = Math.round((horizontal + vertical) / 2)
      }
    }
  }
}

// Gemini 48px 角标背景捕获图，用于还原官方角标的 Alpha 蒙版。
const BG_48_BASE64 =
  ''
/* BG_96 removed - ???? 48 ??? ' */

/**
 * 计算旧版 Gemini 角标的默认位置。
 * 旧版规则：48px 角标距边缘 32px，96px 角标距边缘 64px。
 */
export function getWatermarkParams(imgWidth: number, imgHeight: number, sizeMode: WatermarkSize): {
  size: number
  margin: number
  x: number
  y: number
} {
  const logoSize = sizeMode
  const marginRight = sizeMode === 48 ? 32 : 64
  const marginBottom = sizeMode === 48 ? 32 : 64
  return {
    size: logoSize,
    margin: marginRight,
    x: imgWidth - marginRight - logoSize,
    y: imgHeight - marginBottom - logoSize,
  }
}

/** 根据原图尺寸选择 48px 或 96px 角标蒙版。 */
export function getWatermarkSize(imgWidth: number, imgHeight: number): WatermarkSize {
  return imgWidth > 1024 && imgHeight > 1024 ? 96 : 48
}

/** 获取旧版固定布局中的角标左上角坐标。 */
export function getWatermarkPosition(
  imgWidth: number,
  imgHeight: number,
  size: WatermarkSize
): { x: number; y: number } {
  const { x, y } = getWatermarkParams(imgWidth, imgHeight, size)
  return { x, y }
}

/** 自动定位时要求新版候选至少明显优于旧版候选，避免普通纹理导致误判。 */
const AUTO_POSITION_SCORE_GAIN = 0.08

/**
 * 计算角标 Alpha 与图像亮度的归一化相关度。
 * 白色角标会按 Alpha 比例抬高像素亮度，因此正确位置通常具有更高的正相关。
 */
function scoreWatermarkPosition(
  imageData: Pick<ImageData, 'data' | 'width' | 'height'>,
  alphaMap: Float32Array,
  mapWidth: number,
  mapHeight: number,
  x: number,
  y: number,
): number {
  const { data, width, height } = imageData
  if (x < 0 || y < 0 || x + mapWidth > width || y + mapHeight > height) return Number.NEGATIVE_INFINITY

  let count = 0
  let alphaSum = 0
  let luminanceSum = 0
  let alphaSquareSum = 0
  let luminanceSquareSum = 0
  let productSum = 0

  for (let mapY = 0; mapY < mapHeight; mapY++) {
    for (let mapX = 0; mapX < mapWidth; mapX++) {
      const alpha = alphaMap[mapY * mapWidth + mapX] ?? 0
      const imageIndex = ((y + mapY) * width + x + mapX) * 4
      const luminance = ((data[imageIndex] ?? 0) + (data[imageIndex + 1] ?? 0) + (data[imageIndex + 2] ?? 0)) / 3
      count++
      alphaSum += alpha
      luminanceSum += luminance
      alphaSquareSum += alpha * alpha
      luminanceSquareSum += luminance * luminance
      productSum += alpha * luminance
    }
  }

  const alphaVariance = count * alphaSquareSum - alphaSum * alphaSum
  const luminanceVariance = count * luminanceSquareSum - luminanceSum * luminanceSum
  if (alphaVariance <= 0 || luminanceVariance <= 0) return 0
  return (count * productSum - alphaSum * luminanceSum) / Math.sqrt(alphaVariance * luminanceVariance)
}

/**
 * 在旧版贴边布局与新版内缩布局之间自动选择角标位置。
 * 新版布局的边距约为角标尺寸的两倍；保留旧候选可继续处理历史图片。
 */
export function locateGeminiWatermark(
  imageData: Pick<ImageData, 'data' | 'width' | 'height'>,
  alphaMap: Float32Array,
  mapWidth: number,
  mapHeight: number,
  sizeMode: WatermarkSize,
): { x: number; y: number; alphaScale: number } {
  const legacy = getWatermarkPosition(imageData.width, imageData.height, sizeMode)
  const insetMargin = sizeMode * 2
  const inset = {
    x: imageData.width - insetMargin - mapWidth,
    y: imageData.height - insetMargin - mapHeight,
  }
  const legacyScore = scoreWatermarkPosition(imageData, alphaMap, mapWidth, mapHeight, legacy.x, legacy.y)
  const insetScore = scoreWatermarkPosition(imageData, alphaMap, mapWidth, mapHeight, inset.x, inset.y)
  if (insetScore >= legacyScore + AUTO_POSITION_SCORE_GAIN) {
    // 新版可见角标的实测透明度约为旧版蒙版的 60%，需要同步缩放才能避免反算过度。
    return { ...inset, alphaScale: 0.6 }
  }
  return { ...legacy, alphaScale: sizeMode === 96 ? 0.85 : 1 }
}

/**
 * 从背景捕获图提取 Alpha 蒙版。
 * 捕获图是黑底白标，因此使用最大颜色通道换算透明度。
 */
function alphaMapFromBgCapture(
  bgPixels: Uint8ClampedArray,
  width: number,
  height: number,
  channels: number
): Float32Array {
  const alpha = new Float32Array(width * height)
  for (let i = 0; i < width * height; i++) {
    const off = i * channels
    const r = bgPixels[off] ?? 0
    const g = bgPixels[off + 1] ?? 0
    const b = bgPixels[off + 2] ?? 0
    const gray = Math.max(r, g, b)
    alpha[i] = gray / 255
  }
  return alpha
}

/**
 * 通过反向 Alpha 混合恢复被白色角标覆盖的像素。
 * @param imageData 待处理图像数据
 * @param alphaMap Alpha 蒙版（0~1）
 * @param x 角标左上角 x 坐标
 * @param y 角标左上角 y 坐标
 * @param logoValue 角标颜色值，Gemini 白色角标为 255
 */
export function removeWatermarkReverseAlpha(
  imageData: Pick<ImageData, 'data' | 'width' | 'height'>,
  alphaMap: Float32Array,
  mapWidth: number,
  mapHeight: number,
  x: number,
  y: number,
  logoValue: number = 255,
  alphaScale: number = 1
): void {
  const { data, width, height } = imageData
  const alphaThreshold = 0.002
  const maxAlpha = 0.99

  const x1 = Math.max(0, x)
  const y1 = Math.max(0, y)
  const x2 = Math.min(width, x + mapWidth)
  const y2 = Math.min(height, y + mapHeight)

  for (let py = y1; py < y2; py++) {
    for (let px = x1; px < x2; px++) {
      const alphaIdx = (py - y) * mapWidth + (px - x)
      if (alphaIdx < 0 || alphaIdx >= alphaMap.length) continue
      let alpha = Math.min(alphaMap[alphaIdx] ?? 0, maxAlpha) * alphaScale
      if (alpha < alphaThreshold) continue
      alpha = Math.min(alpha, maxAlpha)
      const oneMinusAlpha = 1 - alpha
      const i = (py * width + px) * 4
      for (let c = 0; c < 3; c++) {
        const watermarked = data[i + c] ?? 0
        const original = (watermarked - alpha * logoValue) / oneMinusAlpha
        if (original < 0) continue
        data[i + c] = Math.round(Math.max(0, Math.min(255, original)))
      }
    }
  }
}

/** 将 48px Alpha 蒙版按双线性插值缩放到目标尺寸。 */
function scaleAlphaMap(
  source: Float32Array,
  srcSize: number,
  dstSize: number
): Float32Array {
  if (srcSize === dstSize) return source
  const out = new Float32Array(dstSize * dstSize)
  for (let ty = 0; ty < dstSize; ty++) {
    for (let tx = 0; tx < dstSize; tx++) {
      const sx = (tx + 0.5) * (srcSize / dstSize) - 0.5
      const sy = (ty + 0.5) * (srcSize / dstSize) - 0.5
      const x0 = Math.max(0, Math.floor(sx))
      const x1 = Math.min(srcSize - 1, x0 + 1)
      const y0 = Math.max(0, Math.floor(sy))
      const y1 = Math.min(srcSize - 1, y0 + 1)
      const fx = sx - x0
      const fy = sy - y0
      const v00 = source[y0 * srcSize + x0] ?? 0
      const v10 = source[y0 * srcSize + x1] ?? 0
      const v01 = source[y1 * srcSize + x0] ?? 0
      const v11 = source[y1 * srcSize + x1] ?? 0
      out[ty * dstSize + tx] =
        (v00 * (1 - fx) + v10 * fx) * (1 - fy) + (v01 * (1 - fx) + v11 * fx) * fy
    }
  }
  return out
}

/**
 * 从内置资源读取 Alpha 蒙版。
 * 96px 模式由 48px 官方蒙版缩放得到，避免重复嵌入大体积资源。
 */
export async function getEmbeddedAlphaMask(
  size: WatermarkSize
): Promise<{ alpha: Float32Array; width: number; height: number }> {
  if (size === 96) {
    const small = await loadAlphaMaskFromUrl(`data:image/png;base64,${BG_48_BASE64}`)
    const scaled = scaleAlphaMap(small.alpha, 48, 96)
    return { alpha: scaled, width: 96, height: 96 }
  }
  const dataUrl = `data:image/png;base64,${BG_48_BASE64}`
  return loadAlphaMaskFromUrl(dataUrl)
}

/**
 * 将蒙版图片解码为 Float32Array。
 */
export async function loadAlphaMaskFromUrl(url: string): Promise<{ alpha: Float32Array; width: number; height: number }> {
  const img = await new Promise<HTMLImageElement>((resolve, reject) => {
    const i = new Image()
    i.crossOrigin = 'anonymous'
    i.onload = () => resolve(i)
    i.onerror = reject
    i.src = url
  })

  const canvas = document.createElement('canvas')
  canvas.width = img.naturalWidth
  canvas.height = img.naturalHeight
  const ctx = canvas.getContext('2d')!
  ctx.drawImage(img, 0, 0)
  const id = ctx.getImageData(0, 0, canvas.width, canvas.height)
  const alpha = alphaMapFromBgCapture(id.data, canvas.width, canvas.height, 4)
  return { alpha, width: canvas.width, height: canvas.height }
}

/** 从 Blob 自动定位并处理 Gemini 可见角标，输出 PNG Blob。 */
export async function removeGeminiWatermarkFromBlob(blob: Blob): Promise<Blob> {
  const dataUrl = await new Promise<string>((resolve, reject) => {
    const r = new FileReader()
    r.onload = () => resolve(r.result as string)
    r.onerror = () => reject(new Error('ERR_READ'))
    r.readAsDataURL(blob)
  })
  const img = await new Promise<HTMLImageElement>((resolve, reject) => {
    const i = new Image()
    i.onload = () => resolve(i)
    i.onerror = reject
    i.src = dataUrl
  })
  const w = img.naturalWidth
  const h = img.naturalHeight
  const baseSize: WatermarkSize = getWatermarkSize(w, h)
  let alphaMap: Float32Array
  let mapW: number
  let mapH: number
  try {
    const loaded = await getEmbeddedAlphaMask(baseSize)
    alphaMap = loaded.alpha
    mapW = loaded.width
    mapH = loaded.height
  } catch {
    alphaMap = createApproxAlphaMap(baseSize)
    mapW = mapH = baseSize
  }
  const canvas = document.createElement('canvas')
  canvas.width = w
  canvas.height = h
  const ctx = canvas.getContext('2d')!
  ctx.drawImage(img, 0, 0)
  const imageData = ctx.getImageData(0, 0, w, h)
  const position = locateGeminiWatermark(imageData, alphaMap, mapW, mapH, baseSize)
  removeWatermarkReverseAlpha(imageData, alphaMap, mapW, mapH, position.x, position.y, 255, position.alphaScale)
  ctx.putImageData(imageData, 0, 0)
  return new Promise((resolve, reject) => {
    canvas.toBlob((b) => (b ? resolve(b) : reject(new Error('ERR_TOBLOB'))), 'image/png', 0.95)
  })
}

/** 当内置蒙版无法加载时，创建近似四角星 Alpha 蒙版作为降级方案。 */
export function createApproxAlphaMap(size: number): Float32Array {
  const alpha = new Float32Array(size * size)
  const cx = size / 2
  const cy = size / 2
  for (let y = 0; y < size; y++) {
    for (let x = 0; x < size; x++) {
      const dx = (x - cx) / cx
      const dy = (y - cy) / cy
      const r = Math.sqrt(dx * dx + dy * dy)
      const angle = Math.atan2(dy, dx)
      const star = Math.abs(Math.sin(angle * 4)) * 0.5 + 0.5
      const radial = Math.max(0, 1 - r * (1.2 - star * 0.3))
      alpha[y * size + x] = Math.min(1, radial * 0.6)
    }
  }
  return alpha
}
