/**
 * XiyueWorkshop 自定义流程：预设节点顺序执行（多图共用同一 JSON 流程）
 */
import {
  applyChromaKey,
...
import {
  ...
  wfMergeStrip,
  wfPadExpand,
} from './xiyueWorkshopWorkflowGridOps'
import {
...
export const XIYUE_WORKSHOP_WORKFLOW_VERSION = 1
...
export const XIYUE_WORKSHOP_WORKFLOW_GRAPH_VERSION = 7
...
export const WORKFLOW_DRAG_INPUT_IMAGE_INDEX = 'application/x-xiyue-workflow-input-image-index'
...
export function serializeWorkflow(nodes: WorkflowNode[]): string {
  const doc: WorkflowDocument = { version: XIYUE_WORKSHOP_WORKFLOW_VERSION, nodes }
...
export function serializeWorkflowGraph(
  ...
  const doc: WorkflowDocument = {
    version: XIYUE_WORKSHOP_WORKFLOW_GRAPH_VERSION,
    nodes,
    edges,
  }
...
export function sanitizeWorkflowPresetFileBase(name: string): string {
  ...
  if (!trimmed) return 'xiyue-workshop-workflow'
  const safe = trimmed.replace(/[/\\?%*:|"<>]/g, '_').replace(/\s+/g, '-')
  return safe || 'xiyue-workshop-workflow'
}
...
export function computeWorkflowRunStrategy(
  ...
  for (const head of sortedInputs) {
    ...
    const steps: WorkflowNode[] = processors.map((g) => {
      const rest = { ...g }
      Reflect.deleteProperty(rest, 'x')
      Reflect.deleteProperty(rest, 'y')
      return rest
    })
...
function parseOneNode(n: unknown, index: number, docVersion: number): GraphNode | null {
  ...
  if (docVersion < XIYUE_WORKSHOP_WORKFLOW_GRAPH_VERSION) {
