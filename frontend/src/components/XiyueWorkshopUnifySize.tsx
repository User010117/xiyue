...
export default function XiyueWorkshopUnifySize() {
  const { t } = useLanguage()
  ...
  const runUnify = async () => {
    ...
    try {
      ...
      message.success(t('xiyueWorkshopUnifySizeSuccess'))
    } catch (e) {
      message.error(t('xiyueWorkshopUnifySizeFailed') + ': ' + String(e))
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <Space direction="vertical" size="large" style={{ width: '100%' }}>
      <Text type="secondary">{t('xiyueWorkshopUnifySizeHint')}</Text>
      ...
      <StashDropZone onStashDrop={(f) => setFiles((prev) => [...prev, f])}>
        ...
        >
          <p className="ant-upload-text">{t('xiyueWorkshopUnifySizeUploadHint')}</p>
        </Dragger>
      </StashDropZone>
        ...
        <>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 12 }}>
              <span>
                <Text type="secondary">{t('xiyueWorkshopUnifySizeCols')}:</Text>
                <InputNumber
                  min={1}
                  max={64}
                  value={cols}
              ...
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
                <Text type="secondary">{t('xiyueWorkshopUnifySizePadH')}:</Text>
                <Select<UnifyPadH>
                  value={padH}
                  onChange={setPadH}
                  style={{ width: 120 }}
                  options={[
                    { value: 'left', label: t('xiyueWorkshopUnifySizePadHLeft') },
                    { value: 'center', label: t('xiyueWorkshopUnifySizePadHCenter') },
                    { value: 'right', label: t('xiyueWorkshopUnifySizePadHRight') },
                  ]}
                />
              </span>
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
                <Text type="secondary">{t('xiyueWorkshopUnifySizePadV')}:</Text>
                <Select<UnifyPadV>
                  value={padV}
                  onChange={setPadV}
                  style={{ width: 120 }}
                  options={[
                    { value: 'top', label: t('xiyueWorkshopUnifySizePadVTop') },
                    { value: 'center', label: t('xiyueWorkshopUnifySizePadVCenter') },
                    { value: 'bottom', label: t('xiyueWorkshopUnifySizePadVBottom') },
                  ]}
                />
              </span>
            </div>
            <Text type="secondary" style={{ fontSize: 12, margin: 0 }}>
              {t('xiyueWorkshopUnifySizePadExplain')}
            </Text>
            <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 12 }}>
            <Button type="primary" loading={loading} onClick={runUnify}>
              {t('xiyueWorkshopUnifySizeRun')}
            </Button>
            {resultUrl && (
              <Button icon={<DownloadOutlined />} onClick={downloadResult}>
                {t('xiyueWorkshopUnifySizeDownload')}
              </Button>
            )}
            </div>
          </div>
          {dims && (
            <Text type="secondary" style={{ fontSize: 12 }}>
              {t('xiyueWorkshopUnifySizeSizeHint', {
                cellW: dims.maxW,
                cellH: dims.maxH,
          ...
          {resultUrl && (
            <div>
              <Text strong style={{ display: 'block', marginBottom: 8 }}>{t('xiyueWorkshopUnifySizeResult')}</Text>
              <div
                style={{
                  padding: 16,
                  background: 'repeating-conic-gradient(#D8CBB5 0% 25%, #D8CBB5 0% 50%) 50% / 16px 16px',
              ...
              >
                <StashableImage
                  src={resultUrl}
                  alt={t('xiyueWorkshopUnifySizeResult')}
