...
import { ExpandOutlined } from './PixelIcons'
...
export default function XiyueWorkshopCustomScale() {
  const { t } = useLanguage()
  ...
  return (
    <Space direction="vertical" size="large" style={{ width: '100%' }}>
      <Text type="secondary">{t('xiyueWorkshopCustomScaleHint')}</Text>

      <Space wrap>
        <span>
          <Text type="secondary">{t('xiyueWorkshopCustomScaleTargetW')}:</Text>
          <InputNumber
            min={1}
            max={4096}
            value={targetW}
        ...
        <span>
          <Text type="secondary">{t('xiyueWorkshopCustomScaleTargetH')}:</Text>
          <InputNumber
            min={1}
            max={4096}
            value={targetH}
      ...
      {file && previewUrl && (
        <>
          <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 12 }}>
            <span>
              <Text type="secondary" style={{ marginRight: 8 }}>{t('xiyueWorkshopMatteAlgorithm')}:</Text>
              <Radio.Group
                value={matteAlgorithm}
                onChange={(e) => setMatteAlgorithm(e.target.value)}
                options={[
                  { label: t('xiyueWorkshopMatteContiguous'), value: 'contiguous' },
                  { label: t('xiyueWorkshopMatteChromaKey'), value: 'chromaKey' },
                  { label: t('xiyueWorkshopMatteChromaKeyAdaptive'), value: 'chromaKeyAdaptive' },
                ]}
              />
            </span>
            {(matteAlgorithm === 'chromaKey' || matteAlgorithm === 'chromaKeyAdaptive') && (
              <span>
                <Text type="secondary" style={{ marginRight: 8 }}>{t('xiyueWorkshopNonContiguousTolerance')}:</Text>
                <InputNumber
                  min={0}
                  max={100}
      ...
      <Space>
        <Button type="primary" loading={loading} onClick={runScale} disabled={!file} icon={<ExpandOutlined />}>
          {t('xiyueWorkshopCustomScaleApply')}
        </Button>
      </Space>
