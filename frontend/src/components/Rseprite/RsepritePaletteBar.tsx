    ...
    >
      <div style={{ marginBottom: 10 }}>
        <Segmented<RsepriteTool>
          ...
          options={[
            { label: t('xiyueWorkshopRsepriteToolPencil'), value: 'pencil' },
            { label: t('xiyueWorkshopRsepriteToolEraser'), value: 'eraser' },
          ]}
        />
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', gap: 20, marginBottom: 10 }}>
        <ColorIndicator
          label={t('xiyueWorkshopRsepriteColorPrimary')}
          rgba={primaryColor}
          active={tool === 'pencil'}
        />
        <ColorIndicator label={t('xiyueWorkshopRsepriteColorSecondary')} rgba={secondaryColor} />
      </div>

      <Typography.Text type="secondary" style={{ display: 'block', fontSize: 11, marginBottom: 8 }}>
        {t('xiyueWorkshopRsepritePalettePresets')}
      </Typography.Text>
        ...
        {presets.map((rgba, i) => {
          ...
          return (
            <button
              key={i}
              type="button"
              title={t('xiyueWorkshopRsepriteSwatchTitle')}
