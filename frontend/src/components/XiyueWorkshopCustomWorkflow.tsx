...
import { CloseOutlined, ExpandOutlined, OrderedListOutlined, PlusOutlined } from './PixelIcons'
...
} from '../lib/xiyueWorkshopWorkflow'
...
const STORAGE_KEY = 'xiyueWorkshopCustomWorkflow.v3'
...
const NODE_LABEL_I18N: Record<WorkflowNodeType, string> = {
  geminiWatermarkRemove: 'xiyueWorkshopWorkflowNode_geminiWatermarkRemove',
  resize: 'xiyueWorkshopWorkflowNode_resize',
  resizeHard: 'xiyueWorkshopWorkflowNode_resizeHard',
  crop: 'xiyueWorkshopWorkflowNode_crop',
  matteContiguous: 'xiyueWorkshopWorkflowNode_matteContiguous',
  matteGlobal: 'xiyueWorkshopWorkflowNode_matteGlobal',
  matteDoubleBackground: 'xiyueWorkshopWorkflowNode_matteDoubleBackground',
  mattePostRepair: 'xiyueWorkshopWorkflowNode_mattePostRepair',
  alphaFrontierErode: 'xiyueWorkshopWorkflowNode_alphaFrontierErode',
  padExpand: 'xiyueWorkshopWorkflowNode_padExpand',
  evenSplitStrip: 'xiyueWorkshopWorkflowNode_evenSplitStrip',
  evenSplitPerCellEdge: 'xiyueWorkshopWorkflowNode_evenSplitPerCellEdge',
  mergeStrip: 'xiyueWorkshopWorkflowNode_mergeStrip',
  simpleStitchVertical: 'xiyueWorkshopWorkflowNode_simpleStitchVertical',
  gridDeleteRow: 'xiyueWorkshopWorkflowNode_gridDeleteRow',
  gridDeleteCol: 'xiyueWorkshopWorkflowNode_gridDeleteCol',
  gridExpandRow: 'xiyueWorkshopWorkflowNode_gridExpandRow',
  gridExpandCol: 'xiyueWorkshopWorkflowNode_gridExpandCol',
  gridFlipRow: 'xiyueWorkshopWorkflowNode_gridFlipRow',
  gridFlipCol: 'xiyueWorkshopWorkflowNode_gridFlipCol',
  gridCopyRow: 'xiyueWorkshopWorkflowNode_gridCopyRow',
  gridCopyCol: 'xiyueWorkshopWorkflowNode_gridCopyCol',
  customGridRearrange: 'xiyueWorkshopWorkflowNode_customGridRearrange',
  workflowInputImage: 'xiyueWorkshopWorkflowNode_workflowInputImage',
}
...
export interface XiyueWorkshopCustomWorkflowProps {
  /** 右键结果图「发送到精细处理」：跳转像素图片 → 精细处理并带入该图 */
  onSendToFineProcess?: (blob: Blob, suggestedFilename: string) => void
}

export default function XiyueWorkshopCustomWorkflow({ onSendToFineProcess }: XiyueWorkshopCustomWorkflowProps = {}) {
  const { t } = useLanguage()
  ...
  const onBlueprintToolPlaceAt = useCallback(
    (action: BlueprintToolPlaceAction, x: number, y: number) => {
      ...
      if (fileItemsRef.current.length === 0) {
        message.warning(t('xiyueWorkshopWorkflowNoFiles'))
        return
      }
  ...
  const executeParsedWorkflow = useCallback(
    async (nodes: GraphNode[], edges: WorkflowEdge[]) => {
      if (nodes.length === 0) {
        message.warning(t('xiyueWorkshopWorkflowNoNodes'))
        return
      }
      ...
      if (!strategyResult.ok) {
        message.warning(t(`xiyueWorkshopWorkflowGraphErr_${strategyResult.reason}`))
        return
      }
      setRunning(true)
      setResults((prev) => {
      ...
      try {
        ...
        setResults(out)
        message.success(t('xiyueWorkshopWorkflowDone'))
      } catch (e) {
        console.error(e)
        message.error(t('xiyueWorkshopWorkflowFailed'))
      } finally {
        setRunning(false)
      }
    },
  ...
  const loadAndRunBpsetiDirect = async (presetId: string, url: string) => {
    ...
    try {
      ...
      message.error(t('xiyueWorkshopWorkflowBpsetLoadFailed'))
    } finally {
  ...
  const saveJsonToFile = () => {
    try {
      ...
      try {
        ...
        message.success(t('xiyueWorkshopWorkflowSavedLast'))
      } catch {
        /* ignore quota */
      }
    } catch {
  ...
  const loadJsonFromStorage = () => {
    try {
      ...
      if (!raw) raw = localStorage.getItem('xiyueWorkshopCustomWorkflow.v2')
      if (!raw) raw = localStorage.getItem('xiyueWorkshopCustomWorkflow.v1')
      if (!raw) {
        message.warning(t('xiyueWorkshopWorkflowNoLast'))
        return
      }
      ...
      message.success(t('xiyueWorkshopWorkflowLoadSuccess'))
    } catch {
      message.error(t('xiyueWorkshopWorkflowLoadFailed'))
    }
  }
    ...
    (n: GraphNode) => {
      ...
      switch (n.type) {
        case 'workflowInputImage': {
          ...
          return (
            <div>
              ...
              <Text style={{ color: '#9aa3b5', fontSize: 11 }}>{t('xiyueWorkshopWorkflowInputImageIndex')}</Text>
              <InputNumber
                min={1}
                max={maxIdx}
                size="small"
        ...
        case 'geminiWatermarkRemove':
          return <Text style={{ color: '#9aa3b5', fontSize: 12 }}>{t('xiyueWorkshopWorkflowNoParams')}</Text>
        case 'resize':
          return (
            <Space direction="vertical" style={{ width: '100%' }} size="small">
              <div>
                <Text style={{ color: '#9aa3b5', fontSize: 11 }}>{t('xiyueWorkshopCustomScaleTargetW')}</Text>
                <InputNumber
                  min={8}
                  max={2048}
                  size="small"
              ...
              <div>
                <Text style={{ color: '#9aa3b5', fontSize: 11 }}>{t('xiyueWorkshopCustomScaleTargetH')}</Text>
                <InputNumber
                  min={8}
                  max={2048}
                  size="small"
              ...
              <Space size="small" wrap>
                <Checkbox
                  ...
                  style={{ color: '#c8d0e0' }}
                >
                  {t('xiyueWorkshopCustomScaleKeepAspect')}
                </Checkbox>
                <Checkbox
                  ...
                  style={{ color: '#c8d0e0' }}
                >
                  {t('xiyueWorkshopWorkflowPixelated')}
                </Checkbox>
              </Space>
            </Space>
          )
              ...
              {(['left', 'top', 'right', 'bottom'] as const).map((k) => (
                <Col span={12} key={k}>
                  <Text style={{ color: '#9aa3b5', fontSize: 11 }}>
                    {t(`xiyueWorkshopWorkflowCrop_${k}`)}
                  </Text>
                  <InputNumber
                    min={0}
                    max={2048}
        ...
        case 'matteGlobal':
          return (
            <Space direction="vertical" style={{ width: '100%' }} size="small">
              <div>
                <Text style={{ color: '#9aa3b5', fontSize: 11 }}>{t('xiyueWorkshopWorkflowTolerance')}</Text>
                <InputNumber
                  min={0}
                  max={150}
                  size="small"
              ...
              <div>
                <Text style={{ color: '#9aa3b5', fontSize: 11 }}>{t('xiyueWorkshopWorkflowFeather')}</Text>
                <InputNumber
                  min={0}
                  max={40}
                  size="small"
        ...
        case 'matteDoubleBackground':
          return (
            <Space direction="vertical" style={{ width: '100%' }} size="small">
              <Text style={{ color: '#8b93a5', fontSize: 11 }}>{t('xiyueWorkshopWorkflowDoubleBgPortsHint')}</Text>
              <div>
                <Text style={{ color: '#9aa3b5', fontSize: 11 }}>{t('xiyueWorkshopWorkflowDoubleBgTolerance')}</Text>
                <InputNumber
                  min={0}
                  max={100}
                  size="small"
              ...
              <div>
                <Text style={{ color: '#9aa3b5', fontSize: 11 }}>{t('xiyueWorkshopWorkflowDoubleBgEdgeContrast')}</Text>
                <InputNumber
                  min={0}
                  max={100}
                  size="small"
        ...
        case 'mattePostRepair':
          return (
            <Text style={{ color: '#8b93a5', fontSize: 11 }}>{t('xiyueWorkshopWorkflowMattePostRepairHint')}</Text>
          )
        case 'alphaFrontierErode':
          return (
            <Space direction="vertical" style={{ width: '100%' }} size="small">
              <Text style={{ color: '#8b93a5', fontSize: 11 }}>{t('xiyueWorkshopWorkflowAlphaFrontierErodeHint')}</Text>
              <div>
                <Text style={{ color: '#9aa3b5', fontSize: 11 }}>{t('xiyueWorkshopWorkflowAlphaFrontierErodeStrength')}</Text>
                <Slider
                  min={0}
                  max={100}
        ...
        case 'padExpand':
          return (
            <Space direction="vertical" style={{ width: '100%' }} size="small">
              {(
                [
                  ['padTop', 'xiyueWorkshopWorkflowPadTop'],
                  ['padRight', 'xiyueWorkshopWorkflowPadRight'],
                  ['padBottom', 'xiyueWorkshopWorkflowPadBottom'],
                  ['padLeft', 'xiyueWorkshopWorkflowPadLeft'],
                ] as const
              ).map(([key, tk]) => (
                <div key={key}>
        ...
        case 'evenSplitStrip':
          return (
            <Space direction="vertical" style={{ width: '100%' }} size="small">
              <div>
                <Text style={{ color: '#9aa3b5', fontSize: 11 }}>{t('xiyueWorkshopWorkflowEvenCols')}</Text>
                <InputNumber
                  min={1}
                  max={32}
                  size="small"
              ...
              <div>
                <Text style={{ color: '#9aa3b5', fontSize: 11 }}>{t('xiyueWorkshopWorkflowEvenRows')}</Text>
                <InputNumber
                  min={1}
                  max={32}
                  size="small"
        ...
        case 'evenSplitPerCellEdge':
          return (
            <Space direction="vertical" style={{ width: '100%' }} size="small">
              <Text style={{ color: '#8b93a5', fontSize: 11 }}>{t('xiyueWorkshopWorkflowEvenSplitPerCellEdgeHint')}</Text>
              <div>
                <Text style={{ color: '#9aa3b5', fontSize: 11 }}>{t('xiyueWorkshopWorkflowEvenCols')}</Text>
                <InputNumber
                  min={1}
              ...
              <div>
                <Text style={{ color: '#9aa3b5', fontSize: 11 }}>{t('xiyueWorkshopWorkflowCellEdgeMode')}</Text>
                <Select
                  size="small"
                  style={INPUT_FULL}
                  value={(p.cellEdgeMode ?? 0) !== 0 ? 1 : 0}
                  options={[
                    { value: 0, label: t('xiyueWorkshopWorkflowCellEdgeModePad') },
                    { value: 1, label: t('xiyueWorkshopWorkflowCellEdgeModeCrop') },
                  ]}
              ...
              {(
                [
                  ['edgeL', 'xiyueWorkshopWorkflowCellEdgeL'],
                  ['edgeT', 'xiyueWorkshopWorkflowCellEdgeT'],
                  ['edgeR', 'xiyueWorkshopWorkflowCellEdgeR'],
                  ['edgeB', 'xiyueWorkshopWorkflowCellEdgeB'],
                ] as const
              ).map(([key, tk]) => (
                <div key={key}>
                  <Text style={{ color: '#9aa3b5', fontSize: 11 }}>{t(tk)}</Text>
        ...
        case 'mergeStrip':
          return (
            <Space direction="vertical" style={{ width: '100%' }} size="small">
              <div>
                <Text style={{ color: '#9aa3b5', fontSize: 11 }}>{t('xiyueWorkshopWorkflowMergeCols')}</Text>
                <InputNumber
                  min={1}
                  max={32}
                  size="small"
              ...
              <div>
                <Text style={{ color: '#9aa3b5', fontSize: 11 }}>{t('xiyueWorkshopWorkflowFrameCount')}</Text>
                <InputNumber
                  min={1}
                  max={4096}
                  size="small"
        ...
        case 'simpleStitchVertical': {
          ...
          return (
            <Space direction="vertical" style={{ width: '100%' }} size="small">
              <div>
                <Text style={{ color: '#9aa3b5', fontSize: 11 }}>{t('xiyueWorkshopWorkflowStitchMode')}</Text>
                <Select
                  size="small"
                  style={{ ...INPUT_FULL, marginTop: 4 }}
                  value={Math.max(0, Math.min(2, Math.round(p.stitchMode ?? 0)))}
                  options={[
                    { value: 0, label: t('xiyueWorkshopWorkflowStitchModeVertical') },
                    { value: 1, label: t('xiyueWorkshopWorkflowStitchModeHorizontal') },
                    { value: 2, label: t('xiyueWorkshopWorkflowStitchModeOverlay') },
                  ]}
                  onChange={(v) => updateNodeParam(id, 'stitchMode', typeof v === 'number' ? v : 0)}
                />
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                <Text style={{ color: '#9aa3b5', fontSize: 11 }}>{t('xiyueWorkshopWorkflowStitchInputSlots', { n: slots })}</Text>
                <Button
                  type="dashed"
                  size="small"
                  icon={<PlusOutlined />}
              ...
              <Text style={{ color: '#7a8499', fontSize: 10 }}>{t('xiyueWorkshopWorkflowSimpleStitchVerticalHint')}</Text>
            </Space>
          )
        }
        case 'gridDeleteRow':
        ...
        case 'gridCopyCol': {
          ...
          const gridCommon = (
            <>
              <div>
                <Text style={{ color: '#9aa3b5', fontSize: 11 }}>{t('xiyueWorkshopWorkflowGridCols')}</Text>
                <InputNumber
                  min={1}
                  max={32}
                  size="small"
              ...
              <div>
                <Text style={{ color: '#9aa3b5', fontSize: 11 }}>{t('xiyueWorkshopWorkflowGridRows')}</Text>
                <InputNumber
                  min={1}
                  max={32}
                  size="small"
              ...
              <Text style={{ color: '#7a8499', fontSize: 10 }}>{t('xiyueWorkshopWorkflowGridCellHint')}</Text>
            </>
          )
          if (n.type === 'gridDeleteRow') {
            return (
              <Space direction="vertical" style={{ width: '100%' }} size="small">
                {gridCommon}
                <div>
                  <Text style={{ color: '#9aa3b5', fontSize: 11 }}>{t('xiyueWorkshopWorkflowRowIndex')}</Text>
                  <InputNumber
                    min={1}
                    max={gr}
                    size="small"
          ...
          if (n.type === 'gridDeleteCol' || n.type === 'gridFlipCol') {
            return (
              <Space direction="vertical" style={{ width: '100%' }} size="small">
                {gridCommon}
                <div>
                  <Text style={{ color: '#9aa3b5', fontSize: 11 }}>{t('xiyueWorkshopWorkflowColIndex')}</Text>
                  <InputNumber
                    min={1}
                    max={gc}
                    size="small"
          ...
          if (n.type === 'gridCopyCol') {
            return (
              <Space direction="vertical" style={{ width: '100%' }} size="small">
                {gridCommon}
                <div>
                  <Text style={{ color: '#9aa3b5', fontSize: 11 }}>{t('xiyueWorkshopWorkflowCopySourceCol')}</Text>
                  <InputNumber
                    min={1}
                    max={gc}
                    size="small"
                ...
                <div>
                  <Text style={{ color: '#9aa3b5', fontSize: 11 }}>{t('xiyueWorkshopWorkflowCopyInsertBeforeCol')}</Text>
                  <InputNumber
                    min={1}
                    max={gc + 1}
                    size="small"
                ...
                <Text style={{ color: '#7a8499', fontSize: 10 }}>{t('xiyueWorkshopWorkflowGridCopyColHint')}</Text>
              </Space>
            )
          }
          if (n.type === 'gridExpandRow') {
            return (
              <Space direction="vertical" style={{ width: '100%' }} size="small">
                {gridCommon}
                <div>
                  <Text style={{ color: '#9aa3b5', fontSize: 11 }}>{t('xiyueWorkshopWorkflowAtRow')}</Text>
                  <InputNumber
                    min={1}
                    max={gr + 1}
                    size="small"
                ...
                <Text style={{ color: '#7a8499', fontSize: 10 }}>{t('xiyueWorkshopWorkflowExpandBandAuto')}</Text>
              </Space>
            )
          }
          if (n.type === 'gridExpandCol') {
            return (
              <Space direction="vertical" style={{ width: '100%' }} size="small">
                {gridCommon}
                <div>
                  <Text style={{ color: '#9aa3b5', fontSize: 11 }}>{t('xiyueWorkshopWorkflowAtCol')}</Text>
                  <InputNumber
                    min={1}
                    max={gc + 1}
                    size="small"
          ...
          if (n.type === 'gridCopyRow') {
            return (
              <Space direction="vertical" style={{ width: '100%' }} size="small">
                {gridCommon}
                <div>
                  <Text style={{ color: '#9aa3b5', fontSize: 11 }}>{t('xiyueWorkshopWorkflowCopySourceRow')}</Text>
                  <InputNumber
                    min={1}
                    max={gr}
                    size="small"
                ...
                <div>
                  <Text style={{ color: '#9aa3b5', fontSize: 11 }}>{t('xiyueWorkshopWorkflowCopyInsertBeforeRow')}</Text>
                  <InputNumber
                    min={1}
                    max={gr + 1}
                    size="small"
                ...
                <Text style={{ color: '#7a8499', fontSize: 10 }}>{t('xiyueWorkshopWorkflowGridCopyRowHint')}</Text>
              </Space>
            )
          }
          return null
        ...
        case 'customGridRearrange': {
          ...
          return (
            <Space direction="vertical" style={{ width: '100%', minWidth: 0 }} size="small">
              <div>
                <Text style={{ color: '#9aa3b5', fontSize: 11 }}>
                  {t('xiyueWorkshopWorkflowRearrangeSplitCols')}
                </Text>
                <InputNumber
                  min={1}
                  max={32}
              ...
              <div>
                <Text style={{ color: '#9aa3b5', fontSize: 11 }}>
                  {t('xiyueWorkshopWorkflowRearrangeSplitRows')}
                </Text>
                <InputNumber
                  min={1}
                  max={32}
              ...
              <div>
                <Text style={{ color: '#9aa3b5', fontSize: 11 }}>
                  {t('xiyueWorkshopWorkflowRearrangeOutRows')}
                </Text>
                <InputNumber
                  min={1}
                  max={64}
              ...
              <div>
                <Text style={{ color: '#9aa3b5', fontSize: 11 }}>
                  {t('xiyueWorkshopWorkflowRearrangeOutCols')}
                </Text>
                <InputNumber
                  min={1}
                  max={64}
              ...
              <Text style={{ color: '#7a8499', fontSize: 10 }}>{t('xiyueWorkshopWorkflowRearrangeGridHint')}</Text>
              <Button
                type="default"
                size="small"
                block
                ...
                style={{ marginTop: 2 }}
              >
                {t('xiyueWorkshopWorkflowRearrangeAutoFill')}
              </Button>
                        ...
                        {rowData.map((val, col) => (
                          ...
                          >
                            <InputNumber
                              ...
                              className="xiyue-blueprint-rearrange-cell"
                              placeholder="0"
                            />
                          </td>
                        ))}
  ...
  return (
    ...
    <Space direction="vertical" size="large" style={{ width: '100%', maxWidth: '100%', marginRight: 'min(336px, calc(100vw - 32px))' }}>
      <Text type="secondary">{t('xiyueWorkshopCustomWorkflowHint')}</Text>
      ...
      <div>
        <Text strong>{t('xiyueWorkshopWorkflowIoTitle')}</Text>
        <div style={{ marginTop: 8 }}>
          <Text style={{ fontSize: 12, color: '#9aa3b5' }}>{t('xiyueWorkshopWorkflowPresetName')}</Text>
          <Input
            value={presetName}
            onChange={(e) => setPresetName(e.target.value)}
            placeholder={t('xiyueWorkshopWorkflowPresetNamePlaceholder')}
            maxLength={120}
            allowClear
        ...
        <Space wrap style={{ marginTop: 8 }}>
          <Button onClick={saveJsonToFile}>{t('xiyueWorkshopWorkflowSaveJson')}</Button>
          <Button
            onClick={() => document.getElementById('xiyue-workflow-json-input')?.click()}
          >
            {t('xiyueWorkshopWorkflowLoadJson')}
          </Button>
          <input
            id="xiyue-workflow-json-input"
            type="file"
            accept=".json,application/json"
            style={{ display: 'none' }}
            onChange={onPickJsonFile}
          />
          <Button onClick={loadJsonFromStorage}>{t('xiyueWorkshopWorkflowLoadLast')}</Button>
          <Button
            type={finishedPresetsOpen ? 'primary' : 'default'}
            onClick={() => setFinishedPresetsOpen((o) => !o)}
          >
            {t('xiyueWorkshopWorkflowLoadFinishedPresets')}
          </Button>
          <Button
            type={directRunPresetsOpen ? 'primary' : 'default'}
            onClick={() => setDirectRunPresetsOpen((o) => !o)}
          >
            {t('xiyueWorkshopWorkflowLoadDirectRunPresets')}
          </Button>
        </Space>
        {finishedPresetsOpen && (
          <Space wrap style={{ marginTop: 8 }}>
      ...
      <div>
        <Text strong>{t('xiyueWorkshopWorkflowImages')}</Text>
        <StashDropZone
          onStashDrop={(file) => {
        ...
        >
          ...
          >
            ...
            <p className="ant-upload-text">{t('xiyueWorkshopWorkflowUploadHint')}</p>
          </Dragger>
          {fileItems.length > 0 && (
            <div
              className="xiyue-workflow-input-thumbs"
              style={{
                display: 'flex',
                flexDirection: 'row',
                flexWrap: 'wrap',
            ...
            >
              {fileItems.map((item, idx) => (
                ...
                >
                  <div
                    draggable
                    title={t('xiyueWorkshopWorkflowDragThumbToBlueprint')}
                    onDragStart={(e) => {
                  ...
                  >
                    ...
                    <Button
                      ...
                      size="small"
                      icon={<PlusOutlined />}
                      aria-label={t('xiyueWorkshopWorkflowAddThumbToBlueprint')}
                      title={t('xiyueWorkshopWorkflowAddThumbToBlueprint')}
      ...
      <Button type="primary" loading={running} onClick={handleRunAll} block>
        {t(
          graphNodes.some((n) => n.type === 'workflowInputImage')
            ? 'xiyueWorkshopWorkflowRunTargeted'
            : 'xiyueWorkshopWorkflowRunAll'
        )}
      </Button>

      {results.length > 0 && (
        <div>
          <Text strong>{t('xiyueWorkshopWorkflowResults')}</Text>
          <Row gutter={[16, 16]} style={{ marginTop: 12 }}>
            {results.map((r) => (
              <Col xs={24} sm={12} key={r.url}>
                <div style={{ border: '1px solid #eee', borderRadius: 0, padding: 8 }}>
                  {onSendToFineProcess ? (
                    <Dropdown
                      menu={{
                        items: [
                          {
                            key: 'fine',
                            label: t('xiyueWorkshopWorkflowSendToFine'),
                            onClick: () => {
                              void (async () => {
                                try {
                  ...
                  >
                    {t('xiyueWorkshopWorkflowDownload')}
                  </Button>
                </div>
              </Col>
            ))}
    ...
    <aside style={PALETTE_FLOAT_OUTER} aria-label={t('xiyueWorkshopWorkflowPalette')}>
      <Text strong style={{ fontSize: 13, display: 'block', flexShrink: 0 }}>
        {t('xiyueWorkshopWorkflowPalette')}
      </Text>
