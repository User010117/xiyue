...
import XiyueWorkshopL7SeamPreview from './XiyueWorkshopL7SeamPreview'

const { Dragger } = Upload
const { Text } = Typography

const IMAGE_ACCEPT = ['.png', '.jpg', '.jpeg', '.webp']
const REARRANGE_STORAGE_KEY = 'xiyue-workshop-customslice-rearrange'
...
export default function XiyueWorkshopCustomSlice() {
  const { t } = useLanguage()
  ...
  const computeL7Preview = async (mode: 'l7' | 'uniform') => {
    ...
    try {
      ...
      message.success(t('xiyueWorkshopCustomSliceL7PreviewDone'))
    } catch (e) {
    ...
    try {
      ...
      if (activeTab === 'custom') {
        ...
        if (
          ...
          message.warning(t('xiyueWorkshopCustomSliceL7NeedPreview'))
          setLoading(false)
          return
        }
        if (activeTab === 'grid' && (previewMeta.cols !== columns || previewMeta.rows !== rows)) {
          message.warning(t('xiyueWorkshopCustomSliceL7PreviewStale'))
          setLoading(false)
          return
        }
        ...
        if (activeTab === 'auto') {
          message.success(
            t('xiyueWorkshopCustomSliceAutoDetected', {
        ...
        message.success(t('xiyueWorkshopCustomSliceAutoDetected', { cols, rows: r0, n: frames.length }))
      } else {
        frames = splitSpriteSheet(img, columns, rows, null)
      }

      if (frames.length === 0) {
        message.warning(t('xiyueWorkshopCustomSliceNoFrames'))
        setLoading(false)
        return
      }
  ...
  const saveRearrangeToTxt = () => {
    ...
    a.click()
    URL.revokeObjectURL(url)
    message.success(t('xiyueWorkshopCustomSliceRearrangeSaveSuccess'))
  }
  ...
  const loadRearrangeFromTxt = () => {
    ...
    input.onchange = async () => {
      ...
      try {
        ...
        message.success(t('xiyueWorkshopCustomSliceRearrangeLoadSuccess'))
      } catch {
        message.error(t('xiyueWorkshopCustomSliceRearrangeLoadFailed'))
      }
    }
    input.click()
  }
  ...
  const runCompose = async () => {
    ...
    try {
      ...
      message.success(t('xiyueWorkshopCustomSliceComposed'))
    } catch (e) {
      message.error(t('xiyueWorkshopCustomSliceComposeFailed') + ': ' + String(e))
    }
  }
  ...
  return (
    <Space direction="vertical" size="large" style={{ width: '100%' }}>
      <Text type="secondary">{t('xiyueWorkshopCustomSliceHint')}</Text>
        ...
        items={[
          {
            ...
            label: (
              <span>
                <ThunderboltOutlined /> {t('xiyueWorkshopCustomSliceAuto')}
              </span>
            ),
            children: (
              <Space direction="vertical">
                <Text type="secondary" style={{ fontSize: 12 }}>
                  {t('xiyueWorkshopCustomSliceAutoHint')}
                </Text>
                <Text type="secondary" style={{ fontSize: 12 }}>
                  {t('xiyueWorkshopCustomSliceAutoDetectHint')}
                </Text>
              </Space>
            ),
          },
          {
            ...
            label: (
              <span>
                <ScissorOutlined /> {t('xiyueWorkshopCustomSliceGrid')}
              </span>
            ),
            children: (
              <Space direction="vertical">
          ...
          },
          {
            ...
            label: (
              <span>
                <ScissorOutlined /> {t('xiyueWorkshopCustomSliceRegions')}
              </span>
            ),
            children: (
              <Space direction="vertical" style={{ width: '100%' }}>
                <Button type="dashed" icon={<PlusOutlined />} onClick={addRegion}>
                  {t('xiyueWorkshopCustomSliceAddRegion')}
                </Button>
                {regions.length === 0 && (
                  <Text type="secondary" style={{ fontSize: 12 }}>
                    {t('xiyueWorkshopCustomSliceAddRegionHint')}
                  </Text>
                )}
      ...
      {spriteFile && spritePreviewUrl && !showL7SeamCanvas && (
        <>
          ...
          <div
            style={{
              padding: 16,
              background: 'repeating-conic-gradient(#D8CBB5 0% 25%, #D8CBB5 0% 50%) 50% / 16px 16px',
              borderRadius: 0,
      ...
      {spriteFile && sliceL7 && activeTab !== 'custom' && (
        <Space direction="vertical" size="small" style={{ width: '100%', maxWidth: 960 }}>
          <Text strong>{t('xiyueWorkshopCustomSliceL7PreviewTitle')}</Text>
          {(!previewMeta || previewMeta.rows !== 1) && (
            <div>
              <Text type="secondary" style={{ fontSize: 12, marginRight: 8 }}>
                {t('xiyueWorkshopCustomSliceL7Band')}
              </Text>
              <Slider
                min={10}
                max={50}
          ...
          <Space wrap>
            <Button loading={loading} onClick={() => void computeL7Preview('l7')}>
              {t('xiyueWorkshopCustomSliceL7PreviewL7')}
            </Button>
            <Button loading={loading} onClick={() => void computeL7Preview('uniform')}>
              {t('xiyueWorkshopCustomSliceL7PreviewUniform')}
            </Button>
          </Space>
          {previewMeta && previewRowStarts && previewColStarts && spritePreviewUrl && (
            <>
              <Text type="secondary" style={{ fontSize: 12, display: 'block' }}>
                {previewMeta.rows === 1
                  ? t('xiyueWorkshopCustomSliceL7PolySeamHint')
                  : t('xiyueWorkshopCustomSliceL7StraightSeams')}
              </Text>
              {previewMeta.rows > 1 && (
                <Text type="warning" style={{ fontSize: 12, display: 'block' }}>
                  {t('xiyueWorkshopCustomSliceL7PolyMultiRowWarn')}
                </Text>
              )}
              {previewMeta.rows === 1 &&
                (l7PolyBendY <= 0 || l7PolyBendY >= previewMeta.h) && (
                  <Text type="secondary" style={{ fontSize: 12, display: 'block' }}>
                    {t('xiyueWorkshopCustomSliceL7PolyBendRangeHint')}
                  </Text>
                )}
              <Text type="secondary" style={{ fontSize: 12, display: 'block', marginBottom: 4 }}>
                {previewMeta.rows === 1
                  ? t('xiyueWorkshopCustomSliceL7PolyDragHint')
                  : t('xiyueWorkshopCustomSliceL7DragHint')}
              </Text>
              <XiyueWorkshopL7SeamPreview
              ...
              <Space direction="vertical" size={8} style={{ width: '100%' }}>
                {previewMeta.rows === 1 &&
                  (() => {
                    ...
                    return (
                      ...
                      >
                        <Text type="secondary" style={{ fontSize: 12 }}>
                          {t('xiyueWorkshopCustomSliceL7PolyParamsTitle')}
                        </Text>
                        <div style={{ width: '100%' }}>
                          <div
                            style={{
                          ...
                          >
                            <Text type="secondary" style={{ fontSize: 11, flex: '1 1 200px' }}>
                              {t('xiyueWorkshopCustomSliceL7PolyBendY')}
                            </Text>
                            <InputNumber
                              size="small"
                              min={0}
                            ...
                            <Text type="secondary" style={{ fontSize: 11 }}>
                              {t('xiyueWorkshopCustomSliceL7PolyBendLimit', { h: previewMeta.h })}
                            </Text>
                          </div>
                          <Slider
                            min={0}
                        ...
                        <div style={{ width: '100%' }}>
                          ...
                          >
                            <Text type="secondary" style={{ fontSize: 11, flex: '1 1 200px' }}>
                              {t('xiyueWorkshopCustomSliceL7PolyDeltaX')}
                            </Text>
                            <InputNumber
                              size="small"
                              min={-dxMax}
                            ...
                            />
                            <Text type="secondary" style={{ fontSize: 11 }}>
                              {t('xiyueWorkshopCustomSliceL7PolyDeltaLimit', { m: dxMax })}
                            </Text>
                          </div>
                          <Slider
                            min={-dxMax}
                ...
                {previewRowStarts.length > 2 && (
                  <Text type="secondary" style={{ fontSize: 12 }}>
                    {t('xiyueWorkshopCustomSliceL7AdjustH')}
                  </Text>
                )}
                    ...
                    <div key={`rh-${i}`} style={{ width: '100%', maxWidth: 640 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 4 }}>
                        <Text type="secondary" style={{ fontSize: 12, minWidth: 72 }}>
                          {t('xiyueWorkshopCustomSliceL7HLine', { i: i + 1 })}
                        </Text>
                        <InputNumber
                          size="small"
                          min={lo}
                ...
                {previewColStarts.length > 2 && (
                  <Text type="secondary" style={{ fontSize: 12, marginTop: 4 }}>
                    {t('xiyueWorkshopCustomSliceL7AdjustV')}
                  </Text>
                )}
                {previewColStarts.slice(1, -1).map((x, idx) => {
                  ...
                  return (
                    <div key={`cv-${j}`} style={{ width: '100%', maxWidth: 640 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 4 }}>
                        <Text type="secondary" style={{ fontSize: 12, minWidth: 72 }}>
                          {t('xiyueWorkshopCustomSliceL7VLine', { j: j + 1 })}
                        </Text>
                        <InputNumber
                          size="small"
                          min={lo}
                  ...
                  (() => {
                    ...
                    return (
                      <Space direction="vertical" size="small" style={{ width: '100%', maxWidth: 640, marginTop: 8 }}>
                        <Text type="secondary" style={{ fontSize: 12 }}>
                          {t('xiyueWorkshopCustomSliceL7JaggedTitle')}
                        </Text>
                        {rowsN === 2 && (
                          <>
                            <Text type="secondary" style={{ fontSize: 11 }}>
                              {t('xiyueWorkshopCustomSliceL7Band01Sync')}
                            </Text>
                            <Slider
                              min={b.min}
                              max={b.max}
                          ...
                          <>
                            ...
                            {Array.from({ length: rowsN - 2 }, (_, k) => k + 2).map((bandIdx) => (
                              <div key={`jag-${bandIdx}`} style={{ width: '100%' }}>
                                <Text type="secondary" style={{ fontSize: 11 }}>
                                  {t('xiyueWorkshopCustomSliceL7BandSegOffset', { n: bandIdx + 1 })}
                                </Text>
                                <Slider
                                  min={b.min}
                                  max={b.max}
      ...
      <Space direction="vertical" size="small" style={{ width: '100%' }}>
        ...
        >
          {sliceL7 && activeTab !== 'custom'
            ? t('xiyueWorkshopCustomSliceL7ApplySplit')
            : activeTab === 'auto'
              ? t('xiyueWorkshopCustomSliceAutoSplit')
              : t('spriteSplit')}
        </Button>
        {zipUrl && (
      ...
      {framePreviewUrls.length > 0 && (
        <>
          <Text strong>{t('imgPreview')} ({t('xiyueWorkshopCustomSliceFrameIndex')}: 1~{frameBlobUrls.length}, 0={t('xiyueWorkshopCustomSliceTransparent')}, {t('xiyueWorkshopCustomSliceFrameIndexFlipHint')})</Text>
          <div
            style={{
              padding: 16,
              background: 'repeating-conic-gradient(#D8CBB5 0% 25%, #D8CBB5 0% 50%) 50% / 16px 16px',
          ...
          <div style={{ marginTop: 16 }}>
            <Text strong>{t('xiyueWorkshopCustomSliceRearrange')}</Text>
            <div style={{ marginTop: 8, display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 16 }}>
              <span>
                <Text type="secondary">{t('xiyueWorkshopCustomSliceRearrangeRows')}:</Text>
                <InputNumber
                  min={1}
                  max={64}
                  value={rearrangeRows}
              ...
              <span>
                <Text type="secondary">{t('xiyueWorkshopCustomSliceRearrangeCols')}:</Text>
                <InputNumber
                  min={1}
                  max={64}
                  value={rearrangeCols}
              ...
              <Button icon={<SaveOutlined />} onClick={saveRearrangeToTxt}>
                {t('xiyueWorkshopCustomSliceRearrangeSaveToTxt')}
              </Button>
              <Button icon={<FolderOpenOutlined />} onClick={loadRearrangeFromTxt}>
                {t('xiyueWorkshopCustomSliceRearrangeLoadFromTxt')}
              </Button>
              <Button type="primary" onClick={runCompose}>
                {t('xiyueWorkshopCustomSliceCompose')}
              </Button>
              {composedUrl && (
                <Button icon={<DownloadOutlined />} onClick={downloadComposed}>
                  {t('xiyueWorkshopCustomSliceDownloadComposed')}
                </Button>
              )}
            </div>
            ...
            <div style={{ marginTop: 16 }}>
              ...
              <div style={{ marginTop: 8, display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 12 }}>
                ...
                <span>
                  <Text type="secondary">{t('xiyueWorkshopCustomSliceExpandDown')}:</Text>
                  <InputNumber
                    min={0}
                    max={128}
                    value={expandDown}
                ...
                <span>
                  <Text type="secondary">{t('xiyueWorkshopCustomSliceExpandLeft')}:</Text>
                  <InputNumber
                    min={0}
                    max={128}
                    value={expandLeft}
                ...
                <span>
                  <Text type="secondary">{t('xiyueWorkshopCustomSliceExpandRight')}:</Text>
                  <InputNumber
                    min={0}
                    max={128}
                    value={expandRight}
              ...
              <div style={{ marginTop: 6, display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 8 }}>
                <Text type="secondary" style={{ fontSize: 12 }}>{t('xiyueWorkshopCustomSliceExpandPreset')}:</Text>
              ...
              <Text type="secondary" style={{ marginTop: 8, fontSize: 12, display: 'block' }}>
                {t('xiyueWorkshopCustomSliceExpandSizeHint', {
                  cellW: paddedCellW,
                  cellH: paddedCellH,
                  outW: composedOutW,
                  outH: composedOutH,
            ...
            {composedUrl && (
              <div style={{ marginTop: 12 }}>
                <Text strong>{t('xiyueWorkshopCustomSliceComposedPreview')}</Text>
                <div
                  style={{
                    marginTop: 8,
                    padding: 16,
