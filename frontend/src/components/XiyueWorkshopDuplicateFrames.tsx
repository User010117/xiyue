...
import { SearchOutlined } from './PixelIcons'
...
export default function XiyueWorkshopDuplicateFrames() {
  const { t } = useLanguage()
  ...
  const runAnalyze = async () => {
    if (!spriteFile || !spritePreviewUrl) {
      message.warning(t('xiyueWorkshopDupFramesNeedImage'))
      return
    }
    setLoading(true)
    thumbUrls.forEach(URL.revokeObjectURL)
    ...
    try {
      ...
      if (groups.length === 0) {
        message.info(t('xiyueWorkshopDupFramesNoDup'))
      } else {
        message.success(
          t('xiyueWorkshopDupFramesFound', { groups: groups.length, frames: cells.length })
        )
      }
    } catch (e) {
      message.error(t('xiyueWorkshopDupFramesFailed') + ': ' + String(e))
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <Space direction="vertical" size="large" style={{ width: '100%' }}>
      <Text type="secondary">{t('xiyueWorkshopDupFramesHint')}</Text>
      ...
      <StashDropZone onStashDrop={(f) => setSpriteFile(f)}>
        ...
        >
          <p className="ant-upload-text">{t('xiyueWorkshopDupFramesUploadHint')}</p>
        </Dragger>
      </StashDropZone>
      ...
      {spriteFile && spritePreviewUrl && (
        <>
          <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 12 }}>
            <span>
              <Text type="secondary">{t('xiyueWorkshopDupFramesCols')}:</Text>
              <InputNumber
                min={1}
                max={64}
                value={columns}
            ...
            <span>
              <Text type="secondary">{t('xiyueWorkshopDupFramesRows')}:</Text>
              <InputNumber
                min={1}
                max={64}
                value={rows}
            ...
            <Button type="primary" icon={<SearchOutlined />} loading={loading} onClick={runAnalyze}>
              {t('xiyueWorkshopDupFramesRun')}
            </Button>
          </div>

          <Text type="secondary" style={{ fontSize: 12 }}>
            {t('xiyueWorkshopDupFramesLogicHint')}
          </Text>
          ...
          {thumbUrls.length > 0 && (
            <div>
              <Text strong style={{ display: 'block', marginBottom: 8 }}>
                {t('xiyueWorkshopDupFramesAllFrames')}
              </Text>
              <div
                style={{
                  display: 'flex',
          ...
          {duplicateGroups.length > 0 && (
            <div>
              <Text strong style={{ display: 'block', marginBottom: 8 }}>
                {t('xiyueWorkshopDupFramesResultTitle')}
              </Text>
                ...
                {duplicateGroups.map((g, gi) => (
                  ...
                  >
                    <Text style={{ display: 'block', marginBottom: 8 }}>
                      {t('xiyueWorkshopDupFramesGroupLine', {
