...
import { DownloadOutlined, EditOutlined, InboxOutlined } from './PixelIcons'
...
export default function ImageResizeStroke({ onSendToFineProcess }: ImageResizeStrokeProps = {}) {
  ...
  const handleExtendBottom = async () => {
    ...
    try {
      const blob = await extendImageBottom(previewBlob)
      setPreviewBlob(blob)
      setPreviewUrl((old) => {
        if (old) URL.revokeObjectURL(old)
        return URL.createObjectURL(blob)
  ...
  const handleOneClickProcess = async () => {
    ...
    try {
      ...
      blob = await extendImageBottom(blob)
      setPreviewBlob(blob)
      ...
      >
        ...
        >
          <p className="ant-upload-drag-icon">
            <InboxOutlined style={{ fontSize: 64, color: '#A94332' }} />
          </p>
