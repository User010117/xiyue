...
/** XiyueWorkshop：单图网格拆分与整图均分功能重叠且易混淆；暂隐藏第三项，改为 true 恢复 */
const SHEET_PRO_GRID_SPLIT_VISIBLE = false

export interface SpriteSheetAdjustProps {
  /**
   * XiyueWorkshop「单图调整 Pro」：整图均分 / 透明拆分（单图网格拆分见 SHEET_PRO_GRID_SPLIT_VISIBLE）→ 格内偏移与边缘裁剪、合成分辨率
   */
...
export default function SpriteSheetAdjust({ integratedSplit = false }: SpriteSheetAdjustProps = {}) {
  ...
  const [previewBg, setPreviewBg] = useState<'checkered' | string>('#D8CBB5')
  const [previewBgColor, setPreviewBgColor] = useState('#D8CBB5')
  ...
  /** XiyueWorkshop：四边数值应用到每一帧（与 GIF 多图合成裁边语义一致，含负数扩边） */
  ...
  /** 仅 XiyueWorkshop 且在调整裁边时启用「固定单格」canvas；否则缩略图走旧版轻量路径，避免卡顿 */
  ...
  const downloadFramesZip = useCallback(async () => {
    ...
    try {
      ...
      message.success(t('xiyueWorkshopSheetProZipSuccess'))
    } catch (e) {
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
            <div
              style={{
                ...
                background: 'repeating-conic-gradient(#D8CBB5 0% 25%, #D8CBB5 0% 50%) 50% / 12px 12px',
                overflow: 'hidden',
                flexShrink: 0,
            ...
            <>
                    ...
                    return (
                      <div
                        ...
                        style={{
                          ...
                          borderRadius: 0,
                          overflow: 'hidden',
                          background: hasFrame ? '#D8CBB5' : 'repeating-conic-gradient(#D8CBB5 0% 25%, #F2E8D5 0% 50%) 50% / 12px 12px',
                          boxSizing: 'border-box',
                  ...
                  <Space style={{ marginBottom: 12 }} wrap>
                    ...
                    <button
                      ...
                      style={{
                        ...
                        border: '1px solid #2D2926',
                        borderRadius: 4,
                        background: playing ? '#A94332' : '#D8CBB5',
                    ...
                    <span style={{ marginLeft: 16, display: 'inline-flex', alignItems: 'center', gap: 8 }}>
                      ...
                      {previewBg === 'solid' && (
                        <ColorPicker
                          ...
                          onChange={(_, hex) => setPreviewBgColor(hex ?? '#D8CBB5')}
                          showText
                          size="small"
                          presets={[
                            { label: '', colors: ['#ffffff', '#D8CBB5', '#D8CBB5', '#808080', '#404040', '#000000'] },
                          ]}
                        />
                      )}
                    </span>
                    ...
                    <div
                      ...
                      style={{
                        ...
                        background: previewBg === 'checkered'
                          ? 'repeating-conic-gradient(#D8CBB5 0% 25%, #D8CBB5 0% 50%) 50% / 16px 16px'
                          : previewBgColor,
                        borderRadius: 0,
                        border: '1px solid #2D2926',
                        display: 'flex',
              ...
              {frameUrls.length > 0 && (
                <div className="sprite-adjust-recombine" style={{ marginTop: 32, paddingTop: 24, borderTop: '1px solid rgba(154,139,120,0.5)' }}>
                  ...
                  <Space wrap>
                    {integratedSplit && (
                      <Button onClick={downloadFramesZip}>{t('xiyueWorkshopSheetProDownloadZip')}</Button>
                    )}
                    <button
                      type="button"
                  ...
                  {recombinedUrl && (
                    <div style={{ marginTop: 12, maxWidth: 480 }}>
                      <img
                        src={recombinedUrl}
                        alt=""
                        style={{ maxWidth: '100%', display: 'block', borderRadius: 0, border: '1px solid #2D2926', imageRendering: 'pixelated' }}
                      />
                    </div>
                  )}
                </div>
