...
import { CaretLeftOutlined, CaretRightOutlined, DeleteOutlined, DownloadOutlined, DragOutlined, FileImageOutlined, LayoutOutlined, PictureOutlined, MergeCellsOutlined, PlusOutlined } from './PixelIcons'
...
export default function GifFrameConverter() {
  ...
  /** 转帧结果的完整动画预览索引；缩略图区仍只显示前 24 张。 */
  const [extractedAnimationIndex, setExtractedAnimationIndex] = useState(0)
  const [gifUrl, setGifUrl] = useState<string | null>(null)

  /** 为所有转出帧创建播放用 URL，避免缩略图上限截断动画。 */
  const extractedAnimationUrls = useMemo(
    () => extractedFrameFiles.map((file) => URL.createObjectURL(file)),
    [extractedFrameFiles],
  )

  const [combineFiles, setCombineFiles] = useState<File[]>([])
  /** 与 combineFiles 同步派生，避免 effect+setState 导致一帧内仍指向旧 URL 时被 revoke 的卡顿与异常解码 */
  const combineInputUrls = useMemo(
    () => combineFiles.map((f) => URL.createObjectURL(f)),
    [combineFiles],
  )
  ...
  useEffect(() => () => extractedAnimationUrls.forEach(URL.revokeObjectURL), [extractedAnimationUrls])

  /** 转帧完成后从第一张开始，并以当前帧间隔自动循环播放。 */
  useEffect(() => {
    setExtractedAnimationIndex(0)
    if (extractedAnimationUrls.length < 2) return
    const timer = window.setInterval(() => {
      setExtractedAnimationIndex((index) => (index + 1) % extractedAnimationUrls.length)
    }, frameDelay)
    return () => window.clearInterval(timer)
  }, [extractedAnimationUrls.length, frameDelay])
  ...
  useEffect(() => {
    return () => combineInputUrls.forEach(URL.revokeObjectURL)
  }, [combineInputUrls])
  ...
  return (
    <Space orientation="vertical" size="large" style={{ width: '100%', paddingTop: 8 }}>
      <Tabs
        activeKey={activeTab}
            ...
            children: (
              <>
                ...
                {extractedFrameUrls.length > 0 && (
                  <>
                    <Text strong style={{ display: 'block', marginTop: 24, marginBottom: 8 }}>帧动画预览</Text>
                    <div
                      style={{
                        padding: 16,
                        background: 'repeating-conic-gradient(#D8CBB5 0% 25%, #D8CBB5 0% 50%) 50% / 16px 16px',
                    ...
                    >
                      <StashableImage
                        src={extractedAnimationUrls[extractedAnimationIndex]}
                        alt={`帧动画 ${extractedAnimationIndex + 1}`}
                        style={{ maxWidth: 320, maxHeight: 240, display: 'block', imageRendering: 'pixelated' }}
                      />
                    </div>
                    <Text type="secondary" style={{ display: 'block', marginTop: 6 }}>{extractedAnimationIndex + 1} / {extractedAnimationUrls.length} · {frameDelay} ms / 帧</Text>
                        ...
                        {frameInputUrls.map((url, i) => (
                          <div
                            ...
                            style={{
                              ...
                              border: frameDragReorderIdx === i ? '2px dashed #A94332' : 'none',
                              borderRadius: 4,
                            }}
                            onDragOver={(e) => {
                              e.preventDefault()
                  ...
                  <div style={{ marginBottom: 16 }}>
                    ...
                    {splitFileUrl && (
                      <div style={{ marginTop: 8 }}>
                        ...
                        <div style={{ marginTop: 4, display: 'inline-block', padding: 8, background: 'repeating-conic-gradient(#D8CBB5 0% 25%, #D8CBB5 0% 50%) 50% / 16px 16px', borderRadius: 4, border: '1px solid #2D2926' }}>
                        ...
                        {combineInputUrls.map((url, i) => (
                          <div
                            ...
                            style={{
                              ...
                              border: dragReorderIdx === i ? '2px dashed #A94332' : 'none',
                              borderRadius: 4,
                            }}
                            onDragOver={(e) => {
                              e.preventDefault()
