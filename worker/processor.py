...
class JobCancelled(RuntimeError):
    """Worker 在安全边界观察到取消标志时抛出的内部异常。"""


def _raise_if_cancelled() -> None:
    """在阶段与帧边界读取 RQ 取消标志，避免继续消耗计算资源。"""
    try:
...
def _get_session():
    """首次实际抠图时再加载 rembg，纯布局检查与 API 启动无需提前加载模型依赖。"""
...
def extract_frames(
    ...
    duration_to_extract = max(0.0, end_sec - start_sec)
    if duration_to_extract <= 0:
        return []

    # 单次 FFmpeg 解码按时间顺序抽帧，避免每帧重新启动进程并重复定位视频。
    output_pattern = output_dir / "frame_%05d.png"
    cmd = [
        "ffmpeg", "-y", "-ss", str(start_sec), "-i", str(video_path),
        "-t", str(duration_to_extract), "-vf", f"fps={fps}",
        "-frames:v", str(max_frames), str(output_pattern),
    ]
    subprocess.run(cmd, capture_output=True, check=True)
    files = sorted(output_dir.glob("frame_*.png"))
    if on_progress:
        on_progress(len(files), len(files))
    return [(path, start_sec + index / fps) for index, path in enumerate(files)]
...
def run_pipeline(job_id: str, video_path: str, output_base: str, temp_base: str, params: dict) -> dict:
    ...
    try:
        ...
        # 运行中删除由 Worker 完成，避免 API 抢先删目录后 Worker 又重新创建文件。
        shutil.rmtree(output_path, ignore_errors=True)
        shutil.rmtree(vpath.parent, ignore_errors=True)
        raise
    finally:
        # 无论成功、失败或取消都释放临时帧，避免磁盘长期累积。
