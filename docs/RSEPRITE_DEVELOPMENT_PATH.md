...
> 当前入口：`XiyueWorkshop` → **Rseprite** → `XiyueWorkshopRseprite.tsx`
> 建议：从 **第 1 步** 起把代码拆到 `frontend/src/components/Rseprite/`，`XiyueWorkshopRseprite.tsx` 只保留壳与组装。
...
- `XiyueWorkshopRseprite.tsx` 里 `useRsepriteState(64, 64)`，开发时在控制台 `console.log(doc)` 验证。
...
- `XiyueWorkshopRseprite.tsx` 内嵌 **第 1 步验收卡片** + 控制台 `[Rseprite Step1]` 日志 + `RESET` 按钮
...
- `XiyueWorkshopRseprite.tsx` 接入首帧首 Cel 的 `ImageData` + 第 2 步说明文案（i18n）
...
- `XiyueWorkshopRseprite.tsx`：首帧首层 `dispatch` 写入
...
- `XiyueWorkshopRseprite.tsx`：`paintRgba` 按工具在 `primaryColor` 与 `ERASER_RGBA` 间切换
...
- `XiyueWorkshopRseprite.tsx`：撤销/重做按钮；`Ctrl/Cmd+Z`、`Ctrl+Y`、`Ctrl/Cmd+Shift+Z`（在输入框外）
...
- `XiyueWorkshopRseprite.tsx`：合成图进 `EditorCanvas`；当前层绘制
...
- `RsepriteTimeline.tsx`、`XiyueWorkshopRseprite.tsx` 接入
...
- `XiyueWorkshopRseprite.tsx`：撤销行内 Checkbox + Slider（5～100%）；单帧时自动禁用。
