# Rseprite（XiyueWorkshop 内嵌）— Aseprite 类功能开发说明

## 定位

- **入口**：XiyueWorkshop 首页卡片「Rseprite」，组件 `frontend/src/components/XiyueWorkshopRseprite.tsx`。
...
| 阶段 | 内容 |
|------|------|
| **M0** | 文档模型定稿、本页占位与宽版外壳（`XiyueWorkshop` 已为 `rseprite` 放宽 `maxWidth`） |
...
frontend/src/components/Rseprite/
  XiyueWorkshopRseprite.tsx    # 壳（可逐步拆为子组件）
  EditorCanvas.tsx
  Timeline.tsx
  LayerPanel.tsx
  PalettePanel.tsx
