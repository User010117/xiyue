...
- **现代像素工作台**：暖色浅底、墨蓝导航、砖红主操作、2px 硬边框与无模糊硬阴影；AI 生成优先，保留分类侧栏、全局搜索、最近使用、收藏和右侧素材暂存抽屉。
- **统一像素图标**：20 个工具、6 个分类和常用动作共用本地 16×16 SVG 网格，不下载字体或大型图片资源。
- **曦月工坊**：旧版高级工坊的自定义缩放、切片、统一尺寸、单图调整等能力完整保留；内部代码名与旧存储键保持兼容。
- **安全与性能**：API Key 默认仅会话保存；上传分块限流、真实媒体校验、RQ 任务恢复和大型工具按需加载。
- **全局快捷键**：`Ctrl/Cmd+K` 打开搜索，`Esc` 返回工作台；移除会与语言和编辑器冲突的 C/H/J 等单键入口。
- **响应式布局**：桌面保留 240px 侧栏和 API 输入/记录双栏；900px 以下改为抽屉导航，390px 下单列且无横向滚动。
...
- **Sprite Sheet 调整**：分割预览、勾选帧、动画预览、预览帧切换（按钮与 **A/D**）、按偏移重组合导出
...
### 开发者与效率（曦月工坊）
- **曦月工坊**：**自定义缩放**、**自定义切片**、**统一尺寸**、**单图调整 Pro** 等本地工具，无需登录。
...
1. 打开 https://github.com/User010117/xiyue/settings/pages
2. 在 **Build and deployment** 下，**Source** 选择 **GitHub Actions**
3. 保存后，下次推送到 `main` 会自动部署

**访问地址：** https://User010117.github.io/xiyue/
...
| [DEV_PLAN_extensions.md](./DEV_PLAN_extensions.md) | 功能扩展规划与已落地项对照 |
